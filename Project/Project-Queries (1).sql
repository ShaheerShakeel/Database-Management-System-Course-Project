...
CREATE VIEW [Transaction Slip View]
AS
	Select Transactions.[Transaction ID],Transactions.[Transaction Name],Transactions.[Transaction Date], Transactions.[Customer ID], Payments.[Amount]
	from Transactions
	LEFT JOIN [Payments] 
		ON Transactions.[Transaction ID]=[Payments].[Customer ID]
GO

Select * from [Transaction Slip View]
----------------Transaction Final Table output for slip---------
...
AS
	Select Reservations.[Reservation ID], Reservations.[Transaction ID], Reservations.[Customer ID], Reservations.[Date In], Reservations.[Date Out], Reservations.[Interval of Stay], Rooms.[Room No], Rooms.[Room Category]
	from Reservations
	JOIN Rooms
		ON Reservations.[Reservation ID] = Rooms.[Room No]
GO
